...
export function DeleteTableWindow({setVisibility}: props){
    ...
    return (
        <div className={'row col-md-5 p-3 popup-window ' + fadeInAnimation} onSubmit={onSubmit}>
            <form>
                <div className="row">
                ...
                <div className="row my-3">
                    ...
                    <select className="col-md-4" onChange={onChange}>
                        <option></option>
                        {
                            allTables.map((table) => <option>Mesa {table.id} - Tamaño {table.size}</option>)
                        }
                    </select>
                </div>
                <div className="row d-flex justify-content-center">
                        <button className="col-md-5" type={'submit'}>Eliminar mesa</button>
                </div>
            </form>
        </div>
    )
