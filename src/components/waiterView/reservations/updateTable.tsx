        ...
        <div className={"row col-md-5 p-3 popup-window  " + fadeInAnimation}>
                ...
                <div className="row my-3">
                    {status === 'Reserved' || status === 'Taken' ? 
                        <h1 className="col-md-11">{ada} por {name}</h1>
                    :  <h1 className="col-md-11">Disponible</h1>}
                </div>
                <div className="row mb-3 d-flex justify-content-center">
                    <button className="col-sm-4" onClick={showChangeWindow}>Cambiar Reserva</button>
                </div>
                ...
                {changeReservation ? 
                    <div className={fadeInAnimation}>
                        <form onSubmit={onSubmit}>
                            ...
                            {newNameVisible ? 
                                <div className="row mb-3">
                                    <label className="col-md-3" htmlFor="name">Nombre: </label>
                                    <input className="col-md-6" required onChange={(event) => setNewName(event.target.value)} name='name' type={'text'}></input>
                                </div>
                            : null}
                            
                            <div className="row mb-3 d-flex justify-content-center">
                                <button className="col-sm-8" type={'submit'}>Guardar cambios</button>
                            </div>
                        </form>
                    </div>
                : null}
