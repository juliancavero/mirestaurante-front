    ...
    <div id="order" className="row d-flex justify-content-center rounded">
      ...
      <div className="col-md-4 row detailedOrderBox p-4 rounded" id="">
        ...
        <div className="d-flex flex-column">
          <button className="btn btn-lg btn-warning my-1 p-2">
            {orderData ? orderData.totalCost : null} €
          </button>
          <button
            className="btn btn-lg btn-success my-1 p-2"
            onClick={createConfirmationWindow}
          >
            Marcar pagado
          </button>
          {/* <button className="btn btn-lg btn-info my-1 p-2">
            Editar pedido
          </button> */}
        </div>
      </div>
      {confirmationVisible ? (
        <AreYouSureWindow
          createConfirmationWindow={createConfirmationWindow}
          setConfirmation={setConfirmed}
          goodOption="Marcar como pagado"
          badOption="Volver"
        />
      ) : null}
    </div>
